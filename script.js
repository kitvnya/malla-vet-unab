const courses = [
  { id: 'biologiaCelular', nombre: 'Biología Celular', semestre: 1, creditos: 6, requisitos: [] },
  { id: 'labBiocel', nombre: 'Lab. Biología Celular', semestre: 1, creditos: 6, requisitos: [] },
  { id: 'zoologia', nombre: 'Zoología', semestre: 1, creditos: 15, requisitos: [] },
  { id: 'matematica', nombre: 'Matemáticas General', semestre: 1, creditos: 13, requisitos: [] },
  { id: 'introVet', nombre: 'Introducción a la Medicina Veterinaria', semestre: 1, creditos: 11, requisitos: [] },
  { id: 'quimica', nombre: 'Química General y Orgánica', semestre: 1, creditos: 14, requisitos: [] },
...
function renderMalla() {
  ...
  for (let s = 1; s <= 10; s++) {
    ...
    t.className = 'semestre-title';
    t.textContent = `Semestre ${s}`;
    col.appendChild(t);

    courses.filter(c => c.semestre === s).forEach(c => {
      const div = document.createElement('div');
      div.className = `course ${estadoCursos[c.id]}`;
      div.dataset.id = c.id;
      div.innerHTML = `<div class="nombre">${c.nombre}</div><div class="creditos">${c.creditos} créditos</div>`;
...
function toggleCurso(id) {
  ...
  courses.forEach(c => {
    if (
      estadoCursos[c.id] === 'locked' &&
      c.requisitos.every(r => estadoCursos[r] === 'completed')
    ) {
